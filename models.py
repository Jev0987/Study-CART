...
class Attention_item(nn.Module):
    def __init__(self, input_dim, dim1, output_dim):
        super(Attention_item, self).__init__()
    ...
    def init_weight(self):
        torch.nn.init.xavier_unormal(self.fc1.weight)
...
class Attention_relation(nn.Module):
    def __init__(self, input_dim, dim1, output_dim):
        super(Attention_relation, self).__init__()
        self.fc1 = nn.Linear(input_dim, dim1)
        self.adding = nn.Linear(dim1, dim1)
        self.fc2 = nn.Linear(dim1, output_dim)
        self.dropout = nn.Dropout(p=0.2)
...
class TransE(nn.Module):
    ...
    def __init__(self, relation_count_list, entity_count_list, device, item_att_model, relation_att_model, norm=2, dim=100, margin=1.0):
        ...
        self.norm = norm
        self.dim = dim
        self.entities_emb_head,self.entities_emb_tail = self._init_entity_emb() #TransE中的头尾entity
    ...
    def _init_entity_emb(self):
        ...
        entities_emb_tail = nn.Embedding(num_embeddings=self.entity_count_list[1] + 1,
                                         embedding_dim=self.dim,padding_idx=self.entity_count_list[1])
        uniform_range = 6 / np.sqrt(self.dim)
    ...
    def _init_relation_emb(self):
        ...
        relations_emb_time = nn.Embedding(num_embeddings=self.relation_count_list[0]+1,
                                          embedding_dim=self.dim, padding_idx=self.relation_count_list[0])
        uniform_range = 6 / np.sqrt(self.dim)
        relations_emb_time.weight.data.uniform_(-uniform_range, uniform_range)

        # -1 to avoid nan for OOV vector
        relations_emb_time.weight.data[:-1, :].div_(
            relations_emb_time.weight.data[:-1, :].norm(p=1, dim=1, keepdim=True))
        ...
        # -1 to avoid nan for OOV vector
        relations_emb_category.weight.data[:-1, :].div_(
            relations_emb_category.weight.data[:-1, :].norm(p=1, dim=1, keepdim=True))
        ...
        # -1 to avoid nan for OOV vector
        relations_emb_clusters.weight.data[:-1, :].div_(relations_emb_clusters.weight.data[:-1, :].norm(p=1, dim=1, keepdim=True)) #.norm()求p范数，dim=0行，dim=1列求p范数，keepdim=True用【】扩起来
    ...
    def loss(self, positive_distances, negative_distances):
        ...
        return self.criterion(positive_distances, negative_distances, target)#用的MarginRankingLoss，loss_val = max(0, -y * (x1 - x2) + margin)
    ...
    def cal_item_context_embedding(self, triplets: torch.LongTensor):
        ...
        relation_emb_value = self.entities_emb_tail(tails) + self.relations_emb_time(
            relation_time) + self.relations_emb_category(relation_category) + \
                             self.relations_emb_clusters(relation_cluster) + self.relations_emb_poi_type(relation_type)
    ...
    def forward2(self, positive_triplets, negative_triplets):
        self.entities_emb_head.weight.data[:-1, :].div_(
            self.entities_emb_head.weight.data[:-1, :].norm(p=2, dim=1, keepdim=True))
        self.entities_emb_tail.weight.data[:-1, :].div_(
            self.entities_emb_tail.weight.data[:-1, :].norm(p=2, dim=1, keepdim=True))
