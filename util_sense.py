...
def rank_items(given_preference, user_sequence, feature_sequence, transE_model, candidate_list, rej_list):
    ...
    relation_1 = asked_cluster_embedding + asked_poi_type_embedding + torch.sum(asked_feature_embedding,
                                                                                dim=0) + time_embedding
    ...
    for i in range(len(history_item_features_list)):
        ...
        item_context_embedding_list.append(item_context_embedding)#尾+所有attribute value
        output = transE_model.item_att_model(concat_embedding) #头cat所有attribute value
